...
$getEvents = function () {
    ...
    $this->viewEvents = $this->events->where('start_date', '=', $this->viewDay);
};
...
<!-- END SETTINGS ------------------------------------------------->
    ...
    <div class="flex flex-col items-left w-full p-2 lg:w-1/3">
        ...
        <livewire:events.events-list-item
            :eventsThisDay="$eventsThisDay"
            :viewDay="$this->viewDay"
            wire:key="day-{{ $eventsThisDay[0]->start_date }}"
        />
        @endforeach
    </div>
    @endif
